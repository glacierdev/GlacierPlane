...
pub async fn list_user_queues(
    ...
    let queues = state.db.get_queues_by_organization_paginated(org_id, limit, offset).await?;
    ...
    for queue in queues {
        ...
        let connected_count = registrations.iter().filter(|a| a.status == "connected").count() as i64;
        let running_count = registrations.iter().filter(|a| a.current_job_id.is_some()).count() as i64;

        let pipeline_name = if let Some(pid) = queue.pipeline_id {
            state.db.get_pipeline_by_id(pid).await.ok().and_then(|p| p.name)
        } else {
            None
        };
...
pub async fn get_user_queue(
    ...
    let (_user, org_id) = get_user_and_org_by_slug(&state, &headers, &org_slug).await?;
    let queue = state.db.get_queue_by_id_and_org(queue_id, org_id).await
    ...
    let mut token_groups: std::collections::HashMap<Uuid, Vec<&crate::db::Agent>> = std::collections::HashMap::new();
...
pub async fn create_user_queue(
    ...
    if payload.name.is_empty() || payload.key.is_empty() {
        return Err(AppError::Http(StatusCode::BAD_REQUEST, "Name and key are required".into()));
    }
    if !payload.key.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
        return Err(AppError::Http(StatusCode::BAD_REQUEST, "Key must contain only alphanumeric characters, hyphens, and underscores".into()));
    }
    if state.db.get_queue_by_key_and_org(&payload.key, org_id).await.is_ok() {
        return Err(AppError::Http(StatusCode::CONFLICT, "A queue with this key already exists".into()));
    }
    if let Some(pipeline_id) = payload.pipeline_id {
        state.db.get_pipeline_by_id_and_org(pipeline_id, org_id).await
            .map_err(|_| AppError::Http(StatusCode::BAD_REQUEST, "Pipeline not found".into()))?;
    }

    let queue = state.db
        .create_queue(user.id, Some(org_id), payload.pipeline_id, &payload.name, &payload.key, payload.description.as_deref(), false)
        .await?;
...
pub async fn update_user_queue(
    ...
    state.db.get_queue_by_id_and_org(queue_id, org_id).await
        .map_err(|_| AppError::Http(StatusCode::NOT_FOUND, "Queue not found".into()))?;
    let queue = state.db.update_queue_by_org(queue_id, org_id, &payload.name, payload.description.as_deref()).await?;
...
pub async fn delete_user_queue(
    ...
    if queue.is_default {
        return Err(AppError::Http(StatusCode::BAD_REQUEST, "Cannot delete default queue".into()));
    }

    let deleted = state.db.delete_queue_by_org(queue_id, org_id).await?;
    if !deleted {
        return Err(AppError::Http(StatusCode::NOT_FOUND, "Queue not found or cannot be deleted".into()));
    }
